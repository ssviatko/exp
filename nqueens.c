...
typedef struct {
	...
	bool is_unique[SOLUTIONS]; // uniqueness flag
} Solutions;
...
typedef struct {
	...
	bool finished; // termination flag
...
void *calc_tf(void *arg)
{
	...
	bool middlestart; // for the sequential algorithm, to determine if we started off with a queen in the "middle" row of column 0 on an odd sized board
	...
	while (!our_td->finished)
	{
		 ...
		 (or less than N/2 + the "middle" row, if we are on an odd sized board) */
		{
			...
			if (our_td->curcol == 0)
				middlestart = ((N % 2 == 1) && (our_td->rowforcol[0] == N / 2)); // == N / 2 instead of N / 2 + 1, since rowforcol[] is zero based
			...
			if (our_td->curcol == N) // if we ran off the right side of the board...
			{
				...
				sol.is_unique[sol.next_sol] = true; // mark it as unique, whether it is tested or not
				int our_solution = sol.next_sol++; // so we know which solution was ours after we unlock the mutex; sol.next_sol may be changed by another thread
				if (PARALLEL)
				...
				if (our_solution > 0) // only if it isn't the first solution found so far
				{
					...
					for (int i = 0; i < our_solution; i++)
					{
							...
							for (int xx = 0; xx < N; xx++)
								for (int yy = 0; yy < N; yy++)
									if (mods[k][xx][yy] != sol.board[i][xx][yy])
										is_same = false;
							if (is_same) // found one that's the same..
							{
						...
						if (!this_is_unique)
						{
							if (PARALLEL)
								pthread_mutex_lock(&sol.sol_lock);
							sol.is_unique[our_solution] = false;
							if (PARALLEL)
								pthread_mutex_unlock(&sol.sol_lock);
							break;
						} // if this_is_unique
...
{
	int res; // result variable for pthread calls
	...
	if (PARALLEL)
	{
		...
		for (int i = N - 1; i >= num_threads; i--)
			sol.next_sol += td[N - i - 1].threadsol;
	}
	else
	{
